...
def extract_video_data(path: str, detector, predictor, verbose: bool = True) -> Optional[np.ndarray]:
    ...
    try:
        video_data = skvideo.io.vread(path, num_frames=env.FRAME_COUNT)
    except:
    ...
    mouth_data_len = len(mouth_data)
    print(mouth_data_len, " mouths found")
    if mouth_data_len < env.FRAME_COUNT:
        if mouth_data_len == 0:
            return None
        more_mouths = [mouth_data[-1] for i in range(env.FRAME_COUNT - mouth_data_len)]
        mouth_data.extend(more_mouths)

    mouth_data = np.array(mouth_data)
    if verbose and bar: bar.finish()

    # out = cv2.VideoWriter('out_video.avi', cv2.VideoWriter_fourcc(*'MJPG'), 25, (360, 288))
    # for md in mouth_data:
    #     out.write(cv2.resize(md, (360, 288)))
    # out.release()
    return mouth_data
